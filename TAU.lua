...
echo -e "\e[34m    @hmndush | @LLLLL3L e[0m"
}
if [ ! -f ./tg ]; then
echo "=============================================="
...
--------------------------------------------------------------------------------------------------------------
function SourceTAU(msg,data) -- بداية العمل
if msg then
...
local keyboard = {
{'تعطيل التواصل 🤹🏻‍♂️️','تفعيل التواصل 🔛'},
...
if text == 'تعطيل التواصل 🤹🏻‍♂️️' and SudoBot(msg) then  
...
usertext = '\n👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(username or 'TAUTEAM')..')'
...
send(msg.chat_id_,0,"⚠┇العضو : {["..data.first_name_.."](T.ME/TAUTEAM)}\n📛┇["..TAU_Msg.."] \n") 
end
end,nil)   
DeleteMessage(msg.chat_id_, {[0] = msg.id_})     
return false
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(arg,data)  
send(msg.chat_id_, msg.id_,'👤┇بواسطه ← ['..utf8.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'TAUTEAM')..') \n🔐┇تـم قفـل الدردشه ')  
end,nil)   
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n👤┇العضو ~⪼ ['..data.first_name_..'](t.me/'..(data.username_ or 'TAUTEAM')..')'
status  = '\n🔰┇تم تنزيله من المميزين في البوت'
send(msg.chat_id_, msg.id_, usertext..status)
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
...
GetWelcome = '✖┇لم يتم وضع ترحيب للمجموعه'
end 
...
if blakrt == 'مميز' and Mod(msg) then
send(msg.chat_id_, msg.id_,'\n👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(text1[3] or 'TAUTEAM')..')'..'\n🔖┇تم رفعه '..text1[2]..' هنا')   
database:sadd(bot_id..'Special:User'..msg.chat_id_,result.id_)  
...
local Json_Info = JSON.decode(TAU)
